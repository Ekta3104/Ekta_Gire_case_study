...
When I first read through this code, it _looks_ fine at a glance — and that's kind of what makes it dangerous. It'll probably work 80% of the time in testing, and then silently break in production in ways that are hard to trace.
  ...
  try {
    ...
    const alerts = rows.map((row) => {
      ...
      const daysUntilStockout =
        dailyRate > 0 ? Math.floor(row.current_stock / dailyRate) : null;
